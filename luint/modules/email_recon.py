...
class EmailReconScanner:
    ...
    def __init__(self, target: str, config: Dict = None, 
        ...
        self.max_pages = self.module_config.get('max_pages', 50)
        self.max_depth = self.module_config.get('max_depth', 3)
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        # Check if we have a valid domain        if not self.domain:
    ...
    def find_emails_web(self) -> Dict[str, Any]:
        ...
        def check_page(url, depth):
            ...
            try:
                ...
                if 'text/html' in content_type:
                    ...
                    new_links = []
                    if depth < self.max_depth and len(checked_pages) < self.max_pages:
                        for link in soup.find_all('a', href=True):
                            href = link['href']
                    ...
                    return url, list(emails_found), new_links
        ...
        email_sources = {}
        pages_to_crawl = [(self.target_url, 0)]  # (url, depth)
        checked_pages = set()

        # Create progress bar
        with progress_bar(f"Searching for emails on {self.target_url}", unit="pages") as progress:
            progress.update(total=self.max_pages)  # Initial total is max_pages
            pages_processed = 0
        ...
        # Use ThreadPoolExecutor for parallel page checks
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for url, depth in pages_to_crawl:
                    futures[executor.submit(check_page, url, depth)] = (url, depth)

                while futures and pages_processed < self.max_pages:
                    ...
                    for future in done:
                        ...
                        try:
                            url, emails, new_links = future.result()
                            ...
                            # Add new links to crawl, if within limits
                            if depth + 1 <= self.max_depth and len(checked_pages) + len(new_links) <= self.max_pages:
                                for link in new_links:
                                    if link not in checked_pages:
                                        futures[executor.submit(check_page, link, depth + 1)] = (link, depth + 1)
                            ...
                            del futures[future]
                            progress.update(advance=1)  # Update progress bar after each page
