...
class SubdomainEnumScanner:
    ...
    def load_wordlist(self) -> List[str]:
        """
        Load subdomain wordlist from file or use a comprehensive default list.
        Supports multiple wordlist paths separated by commas.
        """
        wordlist = set()  # Use set to avoid duplicates

        # Handle multiple wordlist paths
        paths = [path.strip() for path in self.wordlist_path.split(',')]
        ...
        for path in paths:
            try:
                with open(path, 'r') as file:
                    ...
                    self.logger.info(f"Loaded {len(words)} entries from wordlist {path}")
            except (IOError, OSError) as e:
                self.logger.warning(f"Failed to load wordlist from {path}: {str(e)}")

        # Use a comprehensive default wordlist if no file is loaded
        if not wordlist:
            default_wordlist = [
            ...
            wordlist.update(default_wordlist)
    ...
    def virtual_host_discovery(self) -> Dict[str, Any]:
        ...
        if len(subdomains_to_check) > max_vhosts:
            ...
            subdomains_to_check = subdomains_