...
class ServerInfoScanner:
                ...
                with context.wrap_socket(sock, server_hostname=target) as ssock:
                    ...
                    if cert_bin is not None:
                        ...
                        if sig_algo is not None:
                            results['signature_algorithm'] = sig_algo.decode('utf-8')
                        else:
                            results['signature_algorithm'] = 'Unknown'
                        
                        # Validity period
                        not_before_bytes = cert.get_notBefore()
                        not_after_bytes = cert.get_notAfter()
                        
                        if not_before_bytes is not None and not_after_bytes is not None:
                            not_before = datetime.datetime.strptime(not_before_bytes.decode('utf-8'), '%Y%m%d%H%M%SZ')
                            not_after = datetime.datetime.strptime(not_after_bytes.decode('utf-8'), '%Y%m%d%H%M%SZ')
                            ...
                            results['valid_from'] = 'Unknown'
                            results['valid_until'] = 'Unknown'
                            results['is_expired'] = True
                            results['days_until_expiry'] = 0
                        ...
                        if issuer is not None:
                            for i in range(issuer.get_entry_count()):
                                ...
                                if entry is not None and hasattr(entry, 'get_object') and hasattr(entry, 'get_data'):
                                    obj = entry.get_object()
                                    data = entry.get_data()
                                    if obj is not None and data is not None:
                                        issuer_components[obj.decode('utf-8')] = data.decode('utf-8')
                    else:
                        self.logger.warning(f"Could not obtain SSL certificate for {target}:{port}")
    ...
    def _analyze_ssl_tls_security(self, ssl_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'not_after' in ssl_data:
            try:
                expiry_date = datetime.datetime.strptime(ssl_data['not_after'], '%Y-%m-%d %H:%M:%S')
    ...
    def _check_service_vulnerabilities(self, service: str, product: str, version: str) -> List[Dict[str, Any]]:
        ...
        self.logger.debug(f"Checking vulnerabilities for {service}/{product} version {version}")
        
        # Map service name to appropriate category for database lookup
        ...
        if service in service_mappings:
            service_category = service_mappings[service]
        
        # Use the vulnerability database to check for vulnerabilities
        try:
            db_vulnerabilities = self.vuln_db.get_vulnerabilities(service_category, product, version)
            
            # If no exact match found, try searching by product name
            if not db_vulnerabilities:
                self.logger.debug(f"No vulnerabilities found for {service_category}/{product} version {version}, trying keyword search")
                db_vulnerabilities = self.vuln_db.search_vulnerabilities(product)
                
                # Limit keyword search results to most relevant 5
                if db_vulnerabilities and len(db_vulnerabilities) > 5:
                    db_vulnerabilities = db_vulnerabilities[:5]
            
            if db_vulnerabilities:
                self.logger.info(f"Found {len(db_vulnerabilities)} potential vulnerabilities for {product} {version}")
                
                # Convert database vulnerabilities to the expected format
                for vuln in db_vulnerabilities:
                    # Extract and normalize vulnerability data
                    cve_id = vuln.get('vuln_id', vuln.get('id', 'Unknown'))
                    severity = vuln.get('severity', 'medium')
                    ...
                    if severity == 'critical' or severity == 'high':
                        recommendation = f"Urgent: Upgrade {product} immediately"
                    elif severity == 'medium':
                        recommendation = f"Recommended: Update {product} when possible"
                    elif severity == 'low':
                        recommendation = "Consider updating as part of regular maintenance"
                    
                    # Add references to recommendation if available
                    references = vuln.get('references', [])
                    if references:
                        recommendation += ". See " + ", ".join(references[:2])
                    ...
                    # Add references as a separate field if available
                    if references:
        ...
        # Fallback to version analysis if no vulnerabilities found
        if not vulnerabilities and version:
            # Check if version appears outdated based on common versioning patterns
            try:
                # For web servers
                if product.lower() in ['apache', 'nginx', 'iis', 'tomcat'] and service_category == 'http':
                    major_version = int(version.split('.')[0]) if '.' in version else int(version)
                    if product.lower() == 'apache' and major_version < 2:
                        vulnerabilities.append({
                            'cve_id': 'OUTDATED-APACHE',
                            'severity': 'high',
                            'description': f'Apache version {version} is significantly outdated and likely has multiple vulnerabilities',
                            'recommendation': 'Upgrade to Apache 2.4.x or later'
                        })
                    elif product.lower() == 'nginx' and major_version < 1:
                        vulnerabilities.append({
                            'cve_id': 'OUTDATED-NGINX',
                            'severity': 'high',
                            'description': f'Nginx version {version} is significantly outdated and likely has multiple vulnerabilities',
                            'recommendation': 'Upgrade to Nginx 1.20.x or later'
                        })
                
                # For databases
                elif product.lower() in ['mysql', 'mariadb', 'postgresql'] and service_category == 'database':
                    major_version = int(version.split('.')[0]) if '.' in version else int(version)
                    if product.lower() == 'mysql' and major_version < 5:
                        vulnerabilities.append({
                            'cve_id': 'OUTDATED-MYSQL',
                            'severity': 'high',
                            'description': f'MySQL version {version} is significantly outdated and likely has multiple vulnerabilities',
                            'recommendation': 'Upgrade to MySQL 8.0 or later'
                        })
                    elif product.lower() == 'postgresql' and major_version < 10:
                        vulnerabilities.append({
                            'cve_id': 'OUTDATED-POSTGRESQL',
                            'severity': 'medium',
                            'description': f'PostgreSQL version {version} is outdated and may have security vulnerabilities',
                            'recommendation': 'Upgrade to PostgreSQL 13 or later'
                        })
